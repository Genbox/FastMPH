...
internal sealed class Select
{
    ...
    private const int LookupTable1Stride = 8;
    ...
    private void GenerateTable()
    {
        ...
        while (true)
        {
            ...
            _selectTable[selTableIdx] = lookupTable1[(bitsTable[(int)(vecIdx - 1)] * LookupTable1Stride) + (oneIdx - oldPartSum)] + ((vecIdx - 1) << 3); // ((vec_idx - 1) << 3) = ((vec_idx - 1) * 8)
    ...
    private uint Query(uint[] selectTable, uint oneIdx)
    {
        ...
        return lookupTable1[(bitsTable[(int)(vecByteIdx - 1)] * LookupTable1Stride) + (oneIdx - oldPartSum)] + ((vecByteIdx - 1) << 3);
    }
